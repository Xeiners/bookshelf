...
export function BookSheet({ book }: BookSheetProps) {
  ...
  const applyStatus = (status: ReadingStatus) => {
    ...
    notify(t.book.movedTo(t.status[status]), 'like')
  }
  ...
  // Repli de l'API : le résumé n'existe pas dans la langue choisie, on le dit.
  const synopsisFallback =
    book.synopsis && book.synopsisLanguage && book.synopsisLanguage !== language
      ? t.book.synopsisFallback(t.language.inSentence[book.synopsisLanguage])
      : null
    ...
    <div className="fixed inset-0 z-[90]" role="dialog" aria-modal aria-label={book.title}>
      ...
      >
          ...
          <div className="mt-5 flex gap-4" data-sheet-item>
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                ...
                {book.publicationStatus && <Pill>{t.publication[book.publicationStatus]}</Pill>}
                {book.chapters ? <Pill>{t.book.chaptersShort(book.chapters)}</Pill> : null}
                {readingTime && <Pill icon={<Clock size={11} />}>{readingTime}</Pill>}
                {book.pages !== null && <Pill>{t.book.pagesShort(book.pages)}</Pill>}
              </div>
            </div>
            ...
            <div className="flex shrink-0 flex-col gap-2 self-start">
              <button
                ...
                className="glass size-9 rounded-full text-cream/60"
              >
                <X size={16} className="mx-auto" />
              </button>
              {/* Coup de cœur : un titre hors bibliothèque y entre en wishlist. */}
              <FavoriteButton
                active={entry?.favorite ?? false}
                onToggle={() => setFavorite(book, !(entry?.favorite ?? false))}
        ...
        <div className="no-scrollbar min-h-0 flex-1 overflow-y-auto overscroll-contain px-6 pt-5">
          ...
          {synopsisFallback && (
            <p className="mb-2 flex items-center gap-1.5 text-[11px] text-gold/80" data-sheet-item>
              <Languages size={12} />
              {synopsisFallback}
            </p>
          )}
          ...
          <p
            className="text-sm leading-relaxed text-cream/75"
            // Le navigateur (lecteur d'écran, césure) doit connaître la vraie langue du texte.
            lang={book.synopsis ? (book.synopsisLanguage ?? undefined) : undefined}
            data-sheet-item
          >
            {book.synopsis || t.book.noSynopsis}
          </p>
          ...
          {entry?.status === 'reading' && (
            <div className="mt-6 rounded-2xl bg-cream/[0.04] p-4" data-sheet-item>
              <div className="flex items-center justify-between">
                <span className="text-xs text-mist">{t.book.progress}</span>
              ...
              <div className="mt-4 flex gap-2">
                ...
                <Pressable
                  onClick={() => nudgeProgress(PROGRESS_STEP)}
                  aria-label={t.book.stepForward}
                  className="glass grid size-10 place-items-center rounded-full text-cream/70"
                >
                  <Plus size={16} />
                </Pressable>
                <span className="flex-1 self-center text-right text-[11px] text-mist">
                  {book.pages !== null && t.book.pagesProgress(Math.round(book.pages * progress), book.pages)}
                </span>
              </div>
            </div>
          )}
        ...
        {entry?.status === 'read' && (
          // Note personnelle : seulement pour un titre lu, épinglée au-dessus des statuts.
          <div className="shrink-0 px-6 pt-4" data-sheet-item>
            <div className="rounded-2xl bg-cream/[0.04] p-4">
              {/* Hauteur réservée : l'apparition de la note ne fait pas bouger les étoiles sous le doigt. */}
              <div className="flex h-7 items-center justify-between gap-3">
                <span className="text-xs text-mist">{t.book.yourRating}</span>
                {typeof entry.userRating === 'number' && (
                  <span className="font-display text-xl text-gold tabular-nums">{t.book.ratingValue(entry.userRating)}</span>
                )}
              </div>
              <div className="mt-2.5 flex items-center justify-between gap-3">
                <StarRating value={entry.userRating ?? null} onChange={(value) => rate(book.id, value)} />
                {typeof entry.userRating === 'number' && (
                  <button
                    type="button"
                    onClick={() => rate(book.id, null)}
                    className="text-[11px] text-mist transition-colors hover:text-cream"
                  >
                    {t.book.clearRating}
                  </button>
                )}
              </div>
              {/* Deux lignes réservées dans les deux cas : la feuille est ancrée en bas, un
                  texte qui disparaîtrait ferait glisser les étoiles sous le doigt. */}
              <p className="mt-2 min-h-8 text-[11px] leading-4 text-mist/70">
                {typeof entry.userRating === 'number' ? t.book.ratedHint : t.book.rateHint}
              </p>
            </div>
          </div>
        )}
