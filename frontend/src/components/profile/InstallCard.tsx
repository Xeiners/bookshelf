...
export function InstallCard() {
  ...
  const onInstall = async () => {
    ...
    if (outcome === 'accepted') notify(t.install.launched, 'like')
    else if (outcome === 'dismissed') notify(t.install.cancelled, 'nope')
  }
    ...
    return (
      <section ref={rootRef} data-anim className="glass rounded-4xl p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display text-xl leading-tight">{t.install.installedTitle}</h2>
  ...
  return (
    <section ref={rootRef} data-anim className="glass relative overflow-hidden rounded-4xl p-5">
      <div
        data-install-glow
        aria-hidden
        className="pointer-events-none absolute -top-20 -right-14 size-52 rounded-full bg-[radial-gradient(circle,rgb(124_92_255/0.28)_0%,rgb(124_92_255/0.1)_38%,transparent_72%)] opacity-40"
      />
      ...
      <div className="relative">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display text-2xl leading-tight">{t.install.title}</h2>
          ...
          <ol className="mt-4 space-y-2 text-[12px] leading-relaxed text-cream/75">
            <li className="flex gap-2.5">
              ...
              <span className="flex items-center gap-1.5">
                {t.install.iosShareBefore} <Share size={13} className="inline text-glow" />{' '}
                {t.install.iosShareAfter}
              </span>
            </li>
            <li className="flex gap-2.5">
            ...
            <li className="flex gap-2.5">
              ...
              <span>{t.install.iosConfirm}</span>
            </li>
          </ol>
        )}
