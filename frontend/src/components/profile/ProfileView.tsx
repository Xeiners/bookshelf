...
function StatTile({ icon: Icon, label, value, suffix }: StatTileProps) {
  ...
  const valueRef = useCountUp(value, {
    format: (current) => Math.round(current).toLocaleString(t.locale),
  })
...
export function ProfileView() {
  ...
  const memberSince = useMemo(() => {
    ...
    return new Intl.DateTimeFormat(t.locale, { month: 'long', year: 'numeric' }).format(
  ...
  return (
    ...
    >
      ...
      <section className="glass flex items-center gap-5 rounded-4xl p-5" data-anim>
        ...
        <div className="min-w-0">
          <h2 className="font-display text-2xl leading-tight text-cream">
            {t.profile.goalTitle(new Date().getFullYear())}
          </h2>
          <p className="mt-1.5 text-xs leading-relaxed text-mist">
            {goalRatio >= 1
              ? t.profile.goalReached
              : t.profile.goalRemaining(YEARLY_GOAL - stats.byStatus.read)}
          </p>
          {memberSince && (
      ...
      <div className="grid grid-cols-2 gap-3">
        <StatTile icon={BookOpen} label={t.profile.statFinished} value={stats.byStatus.read} />
        <StatTile icon={Flame} label={t.profile.statReading} value={stats.byStatus.reading} />
        <StatTile icon={Layers} label={t.profile.statPages} value={stats.pagesRead} />
        <StatTile icon={Hourglass} label={t.profile.statTime} value={hours} suffix={t.profile.hoursSuffix} />
      </div>
